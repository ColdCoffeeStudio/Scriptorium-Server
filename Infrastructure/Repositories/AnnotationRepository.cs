...
internal sealed class AnnotationRepository(ScriptoriumDbContext context, ILogger<EncyclopediaRepository> logger) : IAnnotationRepository
{
    ...
    public async Task<Result<AnnotationList>> FetchAnnotationFromEncyclopedia(Encyclopedia encyclopedia, CancellationToken cancellationToken)
    {
        ...
        if (canConnect)
        {
            List<Annotation> queryAnnotations = await (
                ...
                where a.encyclopediaId == encyclopedia.Id
                orderby a.id
                select a
            ...
            {
                Result<Domain.Entities.Annotation> tmpAnnotation = Map(annotation, queryThemes, encyclopedia);
